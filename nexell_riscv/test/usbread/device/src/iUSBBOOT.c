...
static void nx_usb_ep0_int_hndlr(USBBOOTSTATUS *pUSBBootStatus)
{
	...
	if(pUSBBootStatus->speed == USB_HIGH) {
		...
		pUOReg->DCSR.DEPIR[CONTROL_EP].DIEPCTL = ((1 << 26) |
				(CONTROL_EP << 11) | (0 << 0));
	} else {
...
static void nx_usb_int_bulkin(USBBOOTSTATUS *pUSBBootStatus)
{
	...
	ecid[0] = *(unsigned int *)0xc0067000;
	ecid[1] = *(unsigned int *)0xc0067004;
	ecid[2] = *(unsigned int *)0xc0067008;
	ecid[3] = *(unsigned int *)0xc006700c;

	printf("Bulk In Function\r\n");
	pUSBBootStatus->up_ptr = (U8*)ecid;
	pUSBBootStatus->up_size = 16;
	pUSBBootStatus->up_addr = ecid;
...
static void nx_usb_transfer(USBBOOTSTATUS *pUSBBootStatus)
{
	...
	if (ep_int & ((1 << CONTROL_EP) << 16)) {
		...
		/* Interrupt Clear */
		pUOReg->DCSR.DEPOR[CONTROL_EP].DOEPINT = ep_int_status;
	}
...
CBOOL iUSBBOOT(struct NX_SecondBootInfo *pTBI)
{
	...
	printf("USB Load Address = 0x%08X Launch Address = 0x%08X,"
			" size = %d bytes\r\n",
