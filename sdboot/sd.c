...
static inline uint8_t spi_xfer(uint8_t d)
{
	...
	do {
        ...
        //        dprintf("[suker] spi_xfer d:%hx, r:%hx\r\n", d, r);
	return r;
}
...
static uint8_t sd_cmd(uint8_t cmd, uint32_t arg, uint8_t crc)
{
	...
	sd_dummy();
	spi_xfer(cmd);
        dprintf("[suker] sd_cmd cmd: %hx  arg: %hx\r\n", cmd, arg);
	spi_xfer(arg >> 24);
	spi_xfer(arg >> 16);
	spi_xfer(arg >> 8);
	spi_xfer(arg);
...
{
	long i;
        kputs("[SUKER] sd_poweron 1");
	REG32(spi, SPI_REG_SCKDIV) = (F_CLK / 300000UL);
        kputs("[SUKER] sd_poweron 2");
	REG32(spi, SPI_REG_CSMODE) = SPI_CSMODE_OFF;
        kputs("[SUKER] sd_poweron 3");
	for (i = 10; i > 0; i--) {
		sd_dummy();
	}
        kputs("[SUKER] sd_poweron 4");
	REG32(spi, SPI_REG_CSMODE) = SPI_CSMODE_AUTO;
        kputs("[SUKER] sd_poweron 5");
}
...
static int sd_cmd0(void)
{
	...
	sd_cmd_end();
        dprintf("[suker] sd_cmd0 end rc: %hx\r\n", rc);
	return rc;
}
